...
class currentDensityPro():
    ''' This class define the main solver for Current Density  without any main graphic GUI window.
    It's designed to work with external JS gui
    '''
    def __init__(self, XsecArr, dXmm, dYmm, lenght,  freq, Currents, temperature, HTC, Gcon, GmxString ):
        print('Creating the currentDensityPro()')
        ...
        self.I = Currents.split(';')  # reading I as array
        self.f = freq
        self.t = temperature
        self.HTC = HTC
        self.Gcon = Gcon
        self.Gmx = np.asarray([gx.split(';') for gx in (GmxString.split('|'))], dtype = float)
    ...
    def console(self, string):
        # Temporary console solution
        #  TODO: Make it call back to JS
        print(string)
